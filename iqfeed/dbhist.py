...
def saveQuote(symbol, instrument, interval, quote):
        ...
        bar_list=Feed.search().filter('term',date=date).filter('term',instrument_id=instrument.id).filter('term',frequency=frequency)
        pass #pass #print			 "close Bar: " + str(dbcontract.id) + " freq ",dbcontract.frequency, " date:" + str(quote['date']) + "date ",date, " open: " + str(quote['open']) + " high:"  + str(quote['high']) + ' low:' + str(quote['low']) + ' close: ' + str(quote['close']) + ' volume:' + str(quote['volume']) 
        if bar_list and bar_list.count() > 0:
            bar=bar_list.execute()[0]
            pass #pass #print			 "found bar id",bar.id
        else:
            pass #print	 symbol
        
            pass #pass #print			 'New Bar: ', quote
            bar=Feed()
...
def    main():
    if len(sys.argv) > 3:
        ...
        pass #print	 "Usage: dbhist.py AAPL 60 100"
