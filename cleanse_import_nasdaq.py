...
def read_csv(filename, exchange):

    inst=comp.getInstrument('^IXIC', 'NASDAQ', 'Index','Index','NASDAQ')
    #nasdaq = getStock(context, sqlctx, inst.id, path_datasets, '^IXIC', start, end)
    
    resource_list=Resource.objects.filter(company_name='NASDAQ')
    ...
    with open(filename, 'rb') as f:
        reader = csv.reader(f)
        for row in reader:
            (Symbol, Name, LastSale, MarketCap, ADRTSO, IPOyear, Sector, Industry, Summary_Quote)=row
...
read_csv('csv/nasdaq_companies.csv', 'NASDAQ')
