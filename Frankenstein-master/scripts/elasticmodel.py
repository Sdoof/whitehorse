...
class Company(DocType):
    ...
    def get_financial_status(self):
        ...
        try:
            result = FinancialStatus.search().filter(company=self).exclude(
...
class CompanyMember(DocType):
    ...
    def save(self, *args, ** kwargs):
        ...
        if not self.user_name and self.user_id:
            try:
                userdefault=UserDefault.search().filter('term',user_id=self.user_id).execute()[0]
                self.user_name=userdefault.first_name + ' ' + userdefault.last_name
            except Exception as e:
                print e
